...
export function enrichRules(rules, { axeMetadata = {}, wcagTags = {}, rationales = {}, manualReasons = {} }) {
  ...
  return rules.map(rule => {
    ...
    const manualReason = manualReasons[rule.id] || null;
    const displayName = metadata.help || rule.id;
    ...
    const resources = [
      ...(isSafeHttpsUrl(rule.helpUrl) ? [{ label: 'Deque University', url: rule.helpUrl }] : [])
    ];

    if (Array.isArray(tags)) {
      tags.forEach(tag => {
        const wcag = wcagTags[tag];
        if (wcag?.w3cURL && isSafeHttpsUrl(wcag.w3cURL)) {
          resources.push({
            label: wcag.title,
            url: wcag.w3cURL
          });
