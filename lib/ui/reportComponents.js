...
export const PrioritySection = (priorityRules, prioritySummary) => {
  ...
  // Internal helper for the natural language list (e.g., "A, B, and C")
...
export const FilterScript = () => `
    (function () {
        // Find the specific container this script lives in
        // This ensures it works whether embedded in a div or standalone
        ...
        // Only target rules within the same parent as this script
        ...
        function applyFilters() {
            ...
            rules.forEach(rule => {
                ...
                if (!filterAll.checked) {
                    // Logic: If any impact box is checked, it must match impact
                    if (activeImpacts.length) {
                        visible = activeImpacts.includes(impact);
                    } else if (!newOnly) {
                        // If nothing is checked and we aren't in 'all' mode, hide everything
                        visible = false;
                    }
