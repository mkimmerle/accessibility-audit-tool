...
export const Header = (siteUrl, auditDate) => `
    <header class="exec-header">
        <div class="exec-header__top">
            ...
            <span class="exec-header__date">${auditDate}</span>
        </div>
        <p class="exec-header__url">
            Analysis for: <strong>${siteUrl}</strong>
        </p>
    </header>
`;
...
/**
 * Updated TrendChart for Executive Report
 * Features: Y-Axis floor (0), Localized short dates, and Design Token integration.
 */
export const TrendChart = (history) => {
    ...
    const width = 1000;
    const height = 180;
    const paddingLeft = 40; // Space for Y-axis "0" label

    // Utility: Format ISO date to M/D/YY
...
export const ImpactSummary = (rules) => {
    ...
    rules.forEach(r => counts[r.impact] = (counts[r.impact] || 0) + r.occurrences.length);
        ...
        <div class="exec-card">
            ...
            <div class="impact-list">
                ${Object.entries(counts).map(([sev, count]) => `
                    <div class="impact-item impact-item--${sev}">
                        <span class="impact-label">${sev.toUpperCase()}</span>
        ...
        <div class="top-pages-list">
            ${topPages.map(p => `
                <div class="page-entry">
                    <a href="${p.url}" target="_blank" rel="noopener" class="page-link">${p.url}</a>
                ...
                ${rules.map(r => {
                    ...
                    return `
                        <tr>
                            ...
                            <td><span class="impact-tag impact--${r.impact}">${r.impact}</span></td>
