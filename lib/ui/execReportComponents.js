...
import { GRADE_THRESHOLDS, BENCHMARK_AVERAGE, getThreshold } from '../io/scoringConfig.js';
...
export const ScoreHero = (grade, score, totalPages, totalWeighted, topRule) => {
    ...
    const benchmarkAverage = BENCHMARK_AVERAGE;
    ...
    // ⚡ Next-grade improvement flag
    const currentIndex = GRADE_THRESHOLDS.findIndex(g => g.grade === grade);
    let nextGradeText = '';
    if (currentIndex > 0) {
        const betterGrade = GRADE_THRESHOLDS[currentIndex - 1];
    ...
    <section class="score-hero">
        <div class="score-hero__main">
            ...
            <div class="score-meta">
                ...
                <p class="score-meta__narrative">${threshold?.description || ''}</p>
                ${topRuleText}
                ${nextGradeText}
            </div>
        </div>
            ...
            <div class="method-item">
                ...
                <ul class="grade-legend">
                    ${GRADE_THRESHOLDS.map(t => `
                        <li><span class="dot dot--${t.grade.toLowerCase()}"></span>
                            <strong>${t.max === Infinity ? '40+' : '0-' + t.max} (${t.grade}):</strong> ${t.label}
                        </li>
                    `).join('')}
                </ul>
            </div>
