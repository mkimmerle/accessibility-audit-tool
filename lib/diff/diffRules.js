...
function getOccurrenceKey(page, ruleId, node) {
  // 1. Normalize URL: remove trailing slashes and hashes
  const cleanPage = page.replace(/\/$/, '').split('#')[0];

  // 2. Normalize Selector: Axe targets can be arrays or strings
...
export function diffRules(rules, resultsDir, friendlyNames = {}, prevAuditFileName = null) {
  ...
  rules.forEach(rule => {
    ...
    rule.occurrences = rule.occurrences.map((o, index) => {
      ...
      // --- DEBUG BLOCK ---
      // If you're still seeing "NEW" badges unexpectedly, this will tell you why.
      if (isNewOccurrence && prevAudit) {
          console.log(`[DIFF DEBUG] New Violation Found:`);
          console.log(`  Rule: ${rule.id}`);
          console.log(`  Key generated: ${key}`);
          console.log(`  Was rule in prev audit? ${prevRuleIds.has(rule.id)}`);
      }
