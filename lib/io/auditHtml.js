 ...
 */
export function writeAuditHtml({ htmlPath, siteUrl, rules, priorityRules, fullyResolvedRules, diffTotals, summary, pagesAudited }) {
  const auditDate = new Date().toLocaleString();

  // --- FORMAT FRIENDLY NAMES FOR RULES ---

  let formattedRules = '';
  if (summary?.topRulesNames && priorityRules.length > 0) {
    // 1. Get the names from priorityRules (which are already sorted by Impact/Count)
    // 2. We only take the top 3 to keep the sentence punchy
    const ruleNames = priorityRules
      .slice(0, 3)
      .map(r => r.displayName || r.id);
    
    // 3. Format with the Oxford Comma
...
<main class="layout-container layout-container--wide results-shell">
    ...
    <section class="priority-callout">
        ...
        ${summary?.topRulesNames ? `
        <p class="priority-tip">
          By focusing on <strong>${formattedRules}</strong>, you can resolve 
          <strong>${summary.violationPercentage}%</strong> of all violations 
          across <strong>${summary.pagePercentage}%</strong> of your audited pages.
        </p>
        ` : ''}
            ...
            <li class="priority-item">
              <a href="#rule-${rule.id}" class="priority-link">
                    ...
                    <span class="priority-details">
                        ...
                        <span class="priority-count">${rule.occurrences.length} page${rule.occurrences.length === 1 ? '' : 's'} affected</span>
                    </span>
                </a>
            </li>
          `;
    ...
    <section class="success-hero">
        <div class="success-content">
            ...
            <p class="success-message">
                ...
                While this is a significant milestone, automated tools typically detect 30–50% of issues. 
                We recommend a manual review of keyboard navigation and screen reader compatibility to reach full compliance.
            </p>
        </div>
    </section>
    ` : ''}
