...
export function writeAuditHtml({ 
  ...
  const manualRuleCount = manualReviewRules.length;

  const totalManualInstances = manualReviewRules.reduce(
    (sum, rule) => sum + rule.occurrences.length,
    0
  );

  const manualPagesSet = new Set();

  manualReviewRules.forEach(rule => {
    rule.occurrences.forEach(o => {
      manualPagesSet.add(o.page);
    });
  });
  
  const manualPageCount = manualPagesSet.size;

  const ruleLabel = `${manualRuleCount} rule${manualRuleCount === 1 ? '' : 's'}`;
  const pageLabel = `${manualPageCount} page${manualPageCount === 1 ? '' : 's'}`;
...
<main class="layout-container layout-container--wide results-shell">
    ...
    <section class="audit-summary-grid">
        ...
        ${UI.SummaryCard('Needs Review', totalManualInstances, 'status--review', '', `${ruleLabel} • ${pageLabel}`)}
  ...
  html += `
    <div id="rules-container">
        <h2 class="rules-title">Confirmed Accessibility Violations</h2>
        <p class="rules-subtitle">Verified WCAG failures requiring remediation.</p>
        ${rules.map(rule => UI.Rule(rule)).join('')}
    </div>`;

  if (manualReviewRules.length > 0) {
    html += UI.ManualReviewSection(manualReviewRules);
  }
