...
import { getHistoryData, IMPACT_WEIGHTS } from './historyDiscovery.js';
import { getGrade } from './scoringConfig.js';
...
export function writeExecHtml({ htmlPath, siteUrl, rawResults, rules }) {
  ...
  const ruleRiskBreakdown = rules.map(r => {
  const weight = weights[r.impact] || 1;
  const uniquePages = new Set(r.occurrences.map(o => o.page)).size;
  const exposurePercent = totalPages > 0 ? (uniquePages / totalPages) : 0;
  
  // Calculate total weighted penalty (Volume)
  const weightedVolume = r.occurrences.length * weight;

  return {
          ...r,
          uniquePages,
          exposurePercent,
          weightedPenalty: weightedVolume,
          riskPercent: Math.round(exposurePercent * 100),
          riskReduction: totalPenalty > 0 ? Math.round((weightedVolume / totalPenalty) * 100) : 0,
          leverageScore: exposurePercent * weight 
      };
  }).sort((a, b) => b.leverageScore - a.leverageScore);

  // Ensure topRule is always defined, or null if there are no rules
  const topRule = ruleRiskBreakdown.length > 0 ? ruleRiskBreakdown[0] : null;

  // Use totalPages > 0 to prevent division by zero or NaN
    ...
    .map(p => {
      ...
      if (p.violations && Array.isArray(p.violations)) {
        p.violations.forEach(v => {
          counts[v.impact] += v.nodes ? v.nodes.length : 0;
        });
      }
...
<body class="exec-report">
    ...
    <main class="exec-container">
        ...
        ${ExecUI.ScoreHero(grade, formattedScore, totalPages, totalPenalty, topRule)}
